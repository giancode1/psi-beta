...
const HEADER_ADMITIDOS = "Cantidad de Admitidos";
    ...
    <div className="grid p-fluid">
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 mb-4">
        <div className="col-span-1">
          <Card className="flex flex-wrap mr-4">
            <CardHeader className="text-lg font-bold ml-2">
              {HEADER_ADMITIDOS} por Provincia
            </CardHeader>
            <CardBody style={{ width: '100%', overflowX: 'scroll' }}>
              <ResponsiveContainer width="100%" height={400}>
                <BarChart
                  data={admitidosPorProvincia}
                ...
                >
                  ...
                  >
                    {admitidosPorProvincia.map((entry, index) => (
        ...
        <div className="col-span-1">
          <Card className="flex flex-wrap mr-4">
            <CardHeader className="text-lg font-bold ml-2">
              {HEADER_ADMITIDOS} por Género
            </CardHeader>
            <CardBody>
              <ResponsiveContainer width="100%" height={400}>
                <PieChart>
                  <Pie
                    data={admitidosPorGenero}
                    dataKey="numero"
                    ...
                    innerRadius={20}
                    fill="#8884d8"
                    label={(props) => renderLabel(admitidosPorGenero, 12, props)}
                  >
                    {admitidosPorGenero.map((entry, index) => (
      ...
      <div className="col-12 xl:col-6 mb-4">
        <Card className="flex flex-wrap mr-4">
          <CardHeader className="text-lg font-bold ml-2">
            {HEADER_ADMITIDOS} por Ciudad
          </CardHeader>
          <CardBody style={{ width: '100%', overflowX: 'scroll' }}>
            <ResponsiveContainer width="100%" height={400}>
              <BarChart
                data={admitidosPorCiudad}
                margin={{ top: 20, right: 30, left: 20, bottom: 95 }}
              >
                <CartesianGrid strokeDasharray="0" vertical={false} stroke="#d0e1fd" />
                <XAxis dataKey="asesorprograma" interval={0} tick={xAxisTickProps} axisLine={axisLineStyle} tickLine={tickLineStyle} />
                ...
                >
                  {admitidosPorCiudad.map((entry, index) => (
                    <Cell key={`cell-${index}`} fill={BAR_COLOR} />
                  ))}
                </Bar>
              </BarChart>
      ...
      <div className="col-12 xl:col-6 mb-4">
        <Card className="flex flex-wrap mr-4">
          <CardHeader className="text-lg font-bold ml-2">
            {HEADER_ADMITIDOS} por Carrera
          </CardHeader>
          <CardBody style={{ width: '100%', overflowX: 'scroll' }}>
            <ResponsiveContainer width="100%" height={400}>
              <BarChart
                data={admitidosPorCarrera}
                margin={{ top: 20, right: 30, left: 20, bottom: 95 }}
              >
                <CartesianGrid strokeDasharray="0" vertical={false} stroke="#d0e1fd" />
                <XAxis dataKey="asesorprograma" interval={0} tick={xAxisTickProps} axisLine={axisLineStyle} tickLine={tickLineStyle} />
                ...
                >
                  {admitidosPorCarrera.map((entry, index) => (
                    <Cell key={`cell-${index}`} fill={BAR_COLOR} />
                  ))}
                </Bar>
              </BarChart>
