...
export default async function getDocList(req, res) {
    try {
        const response = await axios.get(`${process.env.API_NAME}?a=${process.env.AUTH_API_GET_DOCLIST}&key=${process.env.KEY_DATE}`);
