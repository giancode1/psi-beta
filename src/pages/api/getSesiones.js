...
export default async function getSesiones(req, res) {
    try {
        const response = await axios.get(`${process.env.API_NAME}?a=${process.env.AUTH_API_GET_SESION}&key=${process.env.KEY_DATE}`);
        ...
        console.log('Data received:', sesiones);
